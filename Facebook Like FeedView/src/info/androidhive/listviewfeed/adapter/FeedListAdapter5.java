...
public class FeedListAdapter5 extends BaseAdapter {	
	private Activity activity;
	
	private LayoutInflater inflater;
	private List<FeedItem5> feedItems;
	ImageLoader imageLoader = AppController.getInstance().getImageLoader();
	
	public FeedListAdapter5(Activity activity, List<FeedItem5> feedItems) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null)
			convertView = inflater.inflate(R.layout.cat, null);
	...
	//	.findViewById(R.id.feedImage1);
		//TextView ratings = (TextView) convertView
			//	.findViewById(R.id.rating);
		FeedItem5 item = feedItems.get(position);
		foodname.setText(item.getLoc());
...
*/
		// user profile pic
		//((FeedImageView) profilePic).setImageUrl(item.getProfilePic(), imageLoader);
	//	userPic.setImageUrl(item.getuserPic(), imageLoader);
		// Feed image
